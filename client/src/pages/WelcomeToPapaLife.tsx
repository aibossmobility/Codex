...
export default function WelcomeToPapaLife() {
  return <div className="min-h-screen overflow-x-hidden bg-[#f8f0db] text-[#17231c]">
    ...
    <main>
      ...
      <section className="bg-[#145b35] py-16 text-white"><div className="container grid items-center gap-9 md:grid-cols-[280px_1fr]"><img src={brianKeithHillHeadshot} alt="Brian Keith Hill, founder of Papa Life" className="aspect-square w-full rounded-xl object-cover object-top"/><div><p className="text-sm font-black uppercase tracking-[.18em] text-[#f2c230]">About the founder</p><h2 className="mt-3 text-4xl font-extrabold">Brian Keith Hill</h2><p className="mt-5 max-w-3xl text-lg leading-relaxed text-white/85">Brian Keith Hill created Papa Life from lived experience, biblical conviction, and a lifelong commitment to fatherhood. As a father, grandfather, teacher, and mentor, Brian helps men face the places where pride, pain, misunderstanding, and silence have damaged family relationships. He approaches the work as a guide—not a guru—walking beside fathers as they learn to listen, accept responsibility, rebuild trust, and lead through presence.</p></div></div></section>
    ...
    <footer className="bg-[#f2c230] py-8"><div className="container flex flex-col items-center justify-between gap-4 sm:flex-row"><a href="/"><SiteLogo size="md" /></a><div className="flex gap-5 text-sm font-bold"><a href="/privacy-policy">Privacy</a><a href="/terms-of-service">Terms</a><a href="/member-login">Member Login</a></div></div></footer>
  </div>;
}
