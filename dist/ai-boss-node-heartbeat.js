// server/ai-boss-node-heartbeat.ts
import { spawn } from "node:child_process";
import { homedir } from "node:os";
import { join } from "node:path";
var controlUrl = String(process.env.AI_BOSS_CONTROL_URL || "").replace(/\/$/, "");
var token = String(process.env.AI_BOSS_NODE_HEARTBEAT_TOKEN || "").trim();
var nodeId = String(process.env.AI_BOSS_NODE_ID || "brian-mac-mini").trim();
var displayName = String(process.env.AI_BOSS_NODE_NAME || "Brian's Mac mini").trim();
var cdpPort = Number(process.env.AI_BOSS_ZIPSHARE_CDP_PORT || 9333);
var cdpBase = `http://127.0.0.1:${cdpPort}`;
var zipShareProfile = process.env.AI_BOSS_ZIPSHARE_PROFILE || join(homedir(), ".ai-boss-os", "zipshare-browser-profile");
var chromePath = process.env.AI_BOSS_ZIPSHARE_CHROME || "/Applications/Google Chrome 3.app/Contents/MacOS/Google Chrome";
if (!controlUrl || !token) throw new Error("AI_BOSS_CONTROL_URL and AI_BOSS_NODE_HEARTBEAT_TOKEN are required.");
var zipShareMetrics = {};
var zipShareLastSync = "";
function metric(text, code, label) {
...
async function pageTarget() {
  const response = await fetch(`${cdpBase}/json`, { signal: AbortSignal.timeout(3e3) });
  ...
  if (!target) {
    ...
    const retry = await fetch(`${cdpBase}/json`, { signal: AbortSignal.timeout(3e3) });
    const retryTargets = await retry.json();
...
async function evaluateText(wsUrl) {
...
async function refreshZipShareMetrics() {
  try {
    ...
    zipShareLastSync = (/* @__PURE__ */ new Date()).toISOString();
...
async function main() {
  ...
  setInterval(() => heartbeat().catch((error) => console.error(error instanceof Error ? error.message : error)), 3e4);
  setInterval(() => void refreshZipShareMetrics(), 3e5);
}
main().catch((error) => {
