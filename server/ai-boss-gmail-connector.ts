...
async function gmailFetch(fetchImpl: FetchLike, accessToken: string, path: string, init: RequestInit = {}) {
...
function buildMessage(to: string, subject: string, body: string, replyHeaders?: { messageId?: string; references?: string }) {
  const headers = [
    `To: ${to}`,
    `Subject: ${subject}`,
    "MIME-Version: 1.0",
...
export async function executeGmailConnectorAction(action: GmailAction, fetchImpl: FetchLike = fetch) {
  ...
  if (replyMessageId) {
    const original = await gmailFetch(fetchImpl, accessToken,
    ...
    const raw = buildMessage(to, subject, body, { messageId, references });
...
export function createGmailConnectorServer() {
  return http.createServer((req, res) => {
    ...
    req.on("end", async () => {
      try {
        const action = JSON.parse(Buffer.concat(chunks).toString("utf8")) as GmailAction;
